    ...
    def mainLoop(self):
        ...
        for msgid in self.msgs:
            ...
            if not M.getConfirmAddress():
                self.cPrint("can't determine recipient address, skipping",
                            M.msgid)
                continue
...
class Message:
    ...
    def __init__(self, msgid, recipient=None, fullParse=False):
        ...
        if not Q.find_message(self.msgid):
            raise Errors.MessageError(f'{self.msgid} not found!')
        ...
        self.x_primary_address = parseaddr(
            self.msgobj.get('x-primary-address'))[1]
    ...
    def release(self):
        ...
        self.msgobj['X-TMDA-Confirm-Done'] = Cookie.make_confirm_cookie(
            timestamp, pid, 'done')
        ...
        if not self.confirm_accept_address:
            if self.recipient:
                ...
                self.confirm_accept_address = Cookie.make_confirm_address(
                    self.recipient, timestamp, pid, 'accept')
            else:
                return None
    ...
    def summary(self, count=0, total=0, mailto=0):
        ...
        for hdr in Defaults.SUMMARY_HEADERS:
            str += (f"  >> {hdr.capitalize()[:4].rjust(4)}: "
                    f"{Util.decode_header(self.msgobj.get(hdr))}\n")

        if mailto and self.getConfirmAddress():
            str += f'<mailto:{self.confirm_accept_address}>'
        return str
