...
class Auth(Util.Debugable):
    ...
    def init_file(self, file):
        ...
        self.__authfile_allows_cleartext = (Util.getfilemode(file) in
                                            (0o400, 0o600))
        self.__authdict = None
        self.__authdictupdate = None
    ...
    def init_checkpw(self, checkpw):
        ...
        - If this checkpw string contains a space (and therefore arguments) we
          assume that it includes the "true" program in the proper spot for
          that program
        ...
        try:
            if not Util.CanExec(realprogram, self.__ownerID):
                raise ValueError(
                    f"Checkpassword program '{realprogram}' "
                    f"not executable by userid {self.__ownerID}")
        except IOError:
            raise ValueError(f"'{realprogram}' does not exist")
    ...
    def init_remote(self, URI):
        ...
        if authproto not in self.allowed_protocols:
            raise ValueError(f"Protocol '{authproto}' not supported.\n"
                             f"Must be one of {self.allowed_protocols}")
    ...
    def authenticate_plain(self, username, password):
        ...
        try:
            ...
            raise Errors.AuthError(
                f"Unknown authentication type '{self.__authtype}'. Available "
                f"choices for authentication type are "
                f"{self.allowed_authtypes}.")
    ...
    def authenticate_plain_file(self, username, password):
        ...
        if pw == 0:
            ...
            raise Errors.AuthError(
                f"User {username} not found in password file. "
                f"Ensure that this user exists in '{self.__authfile}'.")
        if pw == "":
            self.debug("Blank %s password" % username.lower())
            raise Errors.AuthError(
                f"User {username} is denied login. "
                f"Blank password in file '{self.__authfile}'.")
    ...
    def authenticate_plain_remote(self, username, password):
        ...
        if authhost == '0.0.0.0':
            ...
            if len(ipauthmap) == 0:
                ...
                authdata = ipauthmap.get(
                    self.__localip, '127.0.0.1').split(':')
                authhost = authdata[0]
                if len(authdata) > 1:
                    authport = authdata[1]
            self.__authremote['host'] = authhost
        ...
        if self.__authremote['proto'] == 'imap':
            ...
            if self.__authremote['port']:
                port = int(self.__authremote['port'])
            try:
                M = ldap.initialize("ldap://%s:%s" %
                                    (self.__authremote['host'],
                                     self.__authremote['port']))
                M.simple_bind_s(self.__authremote['dn'] %
    ...
    def __pipecmd(self, command, *strings):
        ...
        process = subprocess.run(
            ...
            input=''.join(strings) if strings else None,
            capture_output=True, encoding='utf-8')
        self.__lastcmderr = process.stderr.strip()
        self.__lastcmdout = process.stdout.strip()
